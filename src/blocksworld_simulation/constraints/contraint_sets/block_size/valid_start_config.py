...
class ValidStartConfig(Constraint):
    """Constraint to validate different blocks start configuration."""
    ...
    def validate(self, state: SimulationState, action: StartAction) -> bool:
        ...
        if len(stack_config) < 2:
            action.set_invalid("different blocks scenario requires at least 2 stacks.")
            return False
        ...
        for stack in stack_config:
            for block in stack:
                ...
                if x_size is None:
                    action.set_invalid("Block missing x_size for different blocks scenario.")
                    return False
                block_sizes.append(x_size)
        # if no blocks are present, return invalid
        if not all_blocks:
            action.set_invalid("Different Blocks requires at least one block.")
            return False
            ...
            if len(stack) > 1:
                for i in range(len(stack) - 1):
                    ...
                    if bottom_block.x_size < top_block.x_size:
                        action.set_invalid("Invalid structure. Wider blocks cannot be on top of narrower blocks.")
                        return False
