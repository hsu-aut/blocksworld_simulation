...
from blocksworld_simulation.simulation.simulation_action import StackAction
from blocksworld_simulation.simulation.simulation_state import SimulationState


class BlockBelowWider(Constraint):
    ...
    def validate(self, state: SimulationState, action: StackAction) -> Tuple[bool, str]:
        ...
        if target_block_width <= block_width:
            action.set_invalid("Block cannot be stacked on a smaller or equally wide block.")
            return False
